...
func NewMetadataManager(db *gorm.DB, geocoder *Geocoder, workers int, intervalMinutes int) *MetadataManager {
	mm := &MetadataManager{
		db:       db,
		geocoder: geocoder,
		workers:  workers,
...
func (mm *MetadataManager) processUnextracted() {
	...
	jobs := make(chan ImageFile, total)
...
func (mm *MetadataManager) upsertBatch(batch []*ImageMetadata) {
